    ...
    public boolean makeText(String string, String _font, int prefered_color, int fade_mode, float x, float y, float time_to_spend) {
        ...
        switch (prefered_color) {
            ...
            case COLOR_PREF.GREEN: // GREEN prefered
                ...
                G[i] = 1.0f;
                break;
            case COLOR_PREF.WHITE: // BLUE prefered
                R[i] = r.nextFloat();
                G[i] = r.nextFloat();
                B[i] = 1.0f;
                break;
        ...
        if (spriteBatch != null) {
            ...
            for (int j=0; j < i; j++) {
                ...
                if ((time[j] == ETERNAL ||time[j] > 0) && (fade_mode[j] == STYLE.ROUND || blend[j] > 0.15)) {
                    ...
                    // we draw .. finally
                    spriteBatch.draw(back[j], X[j]-(margin/2)-30, Y[j]-(margin/2)-30, font_width[j], font_height[j]);
                    font[j].setColor(R[j], G[j], B[j], blend[j]);
                    font[j].draw(spriteBatch, string[j], X[j]-50, Y[j]-50);
                }
            }
            spriteBatch.end();
        }
